...
export default function ResultsPage() {
  ...
  return (
    <PortalLayout title="Result Slip" subtitle={university.semester}>
      ...
      <div
        id="result-slip"
        className="bg-white shadow-sm rounded-lg overflow-hidden max-w-4xl mx-auto"
      >
        ...
        <div className="px-6 py-4 grid grid-cols-2 gap-x-10 border-b border-gray-200">
          ...
          <div className="space-y-2.5">
            {[
              ...
              <div key={label} className="flex gap-3 text-sm">
                <span className="font-bold text-gray-700 w-28 shrink-0">
                  {label}
                </span>
          ...
          <div className="space-y-2.5">
            {[
              ...
              <div key={label} className="flex gap-3 text-sm">
                <span className="font-bold text-gray-700 w-32 shrink-0">
                  {label}
                </span>
                <span className="text-gray-600">{value}</span>
              </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#1a237e] text-white">
                ...
                <th className="text-center px-3 py-2.5 font-semibold text-xs">
                  Credit
                </th>
                <th className="text-center px-3 py-2.5 font-semibold text-xs">
                  Grade
                ...
                <th className="text-left px-3 py-2.5 font-semibold text-xs">
                  Remark
                </th>
              </tr>
            </thead>
            <tbody>
              {courses.map((c, i) => (
                <tr
                  key={c.code}
                  className={i % 2 === 0 ? "bg-white" : "bg-[#dbeafe]"}
                >
                  <td className="px-4 py-2.5 font-mono text-xs font-semibold text-gray-600">
                    {c.code}
                  </td>
                  <td className="px-4 py-2.5 text-xs text-gray-800">
                    {c.name}
                  </td>
                  <td className="text-center px-3 py-2.5 text-xs font-semibold text-gray-700">
                    {c.credit}
                  </td>
                  <td className="text-center px-3 py-2.5 text-xs font-bold text-gray-800">
                    {c.grade}
                  </td>
              ...
              <tr className="bg-[#1a237e]/10 border-t-2 border-[#1a237e]/30">
                <td
                  colSpan={2}
                  className="px-4 py-2.5 font-bold text-xs text-[#1a237e]"
                >
                  TOTALS / SUMMARY
                </td>
                ...
                <td className="text-center px-3 py-2.5 font-bold text-xs text-amber-700">
                  {avgScore} avg
                </td>
                <td className="text-center px-3 py-2.5 font-bold text-xs text-[#1a237e]">
                  {gpa} GPA
        ...
        <div className="px-4">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className={`h-6 ${i % 2 === 0 ? "bg-white" : "bg-[#dbeafe]/40"}`}
            />
          ))}
        </div>
        ...
        <div className="px-8 py-4 border-t border-gray-200">
          <p className="text-xs text-gray-600 italic leading-relaxed">
            <strong>Disclaimer:</strong> This Result Slip is only a notification
            of results to students and not a replacement for the official
            academic transcript. For any inquiry on grades, kindly contact your
            academic advisor or the Head of your Department within 14 days of
            publication.
          </p>
        </div>
      ...
      <div className="mt-5 bg-white rounded-lg shadow-sm overflow-hidden max-w-4xl mx-auto no-print">
        <div className="bg-[#1a237e] text-white px-5 py-2.5">
          <h3 className="font-bold text-sm">
            Nile University Grading Scale (5.0 System)
          </h3>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              {["Score Range", "Letter Grade", "Grade Points", "Remark"].map(
                (h) => (
                  <th
                    key={h}
                    className="text-left px-5 py-2 text-xs text-gray-500 font-semibold uppercase tracking-wide"
                  >
                    {h}
                  </th>
                ),
          ...
          <tbody>
            {gradeScale.map((row, i) => (
              <tr
                key={row.letter}
                className={`border-b border-gray-50 ${i % 2 === 0 ? "bg-white" : "bg-[#dbeafe]/30"}`}
              >
                ...
                <td className="px-5 py-2">
                  <span
                    className={`font-bold text-xs ${row.letter === "A" ? "text-green-700" : row.letter === "B" ? "text-blue-700" : row.letter === "C" ? "text-amber-700" : "text-red-700"}`}
                  >
                    {row.letter}
                  </span>
                </td>
                <td className="px-5 py-2 text-xs font-medium text-gray-700">
