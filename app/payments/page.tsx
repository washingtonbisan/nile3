...
export default function PaymentsPage() {
  return (
    <PortalLayout
      ...
      subtitle={`${fees.academicYear} Fee Account`}
    >
      {/* Paid status banner */}
      {fees.balance === 0 && (
        <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-lg px-5 py-3.5 mb-5">
          ...
          <div>
            <p className="font-semibold text-green-800 text-sm">
              Account is clear — All fees paid
            </p>
            <p className="text-xs text-green-600 mt-0.5">
              Full payment of ₦{fees.totalPaid.toLocaleString()} confirmed for{" "}
              {fees.academicYear} academic session.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
        {[
          {
            label: "Total Billed",
            value: `₦${fees.totalBilled.toLocaleString()}`,
            bg: "bg-[#1a237e] text-white",
          },
          {
            label: "Total Paid",
            value: `₦${fees.totalPaid.toLocaleString()}`,
          ...
          },
          {
            ...
            bg: "bg-white text-[#1a237e] border border-gray-200",
          },
        ].map(({ label, value, bg }) => (
        ...
        <div className="bg-white rounded-lg shadow-sm overflow-hidden">
          <div className="bg-[#1a237e] text-white px-5 py-3 flex items-center justify-between">
            <h3 className="font-bold text-sm">
              Fee Statement — {fees.academicYear}
            </h3>
            <div className="flex gap-2">
              <button
                onClick={() => window.print()}
                className="no-print p-1.5 text-white/60 hover:text-white rounded hover:bg-white/10"
              >
                <Printer size={15} />
              </button>
            </div>
          ...
          <div className="px-5 py-2.5 bg-gray-50 border-b border-gray-100 text-xs text-gray-500 flex flex-wrap gap-x-4">
            <span>
              <strong className="text-gray-700">Student:</strong> {student.name}
            </span>
            <span>
              <strong className="text-gray-700">ID:</strong> {student.studentId}
            </span>
            <span>
          ...
          <div className="flex items-center justify-between px-5 py-3 bg-[#1a237e] text-white">
            ...
            <p className="font-bold text-lg">
              ₦{fees.totalBilled.toLocaleString()}
            </p>
          </div>
          ...
          <div className="flex items-center justify-between px-5 py-2.5 bg-gray-50 border-t border-gray-100">
            ...
            <p
              className={`font-bold ${fees.balance === 0 ? "text-green-600" : "text-red-600"}`}
            >
              {fees.balance === 0 ? "NIL" : `₦${fees.balance.toLocaleString()}`}
            </p>
          </div>
